...
export const api = {
  ...
  async updateOrder(id: string, data: { status?: string; tracking_number?: string; tracking_url?: string }) {
    return request<Order>(`/v1/orders/${id}`, {
      method: 'PATCH',
  ...
  async createVariant(productId: string, data: { sku: string; title: string; price_cents: number; image_url?: string }) {
  ...
  async updateVariant(productId: string, variantId: string, data: { sku?: string; title?: string; price_cents?: number; image_url?: string | null }) {
  ...
  async uploadImage(file: File) {
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new ApiError(err.error?.code || 'unknown', res.status, err.error?.message || 'Upload failed');
    }
